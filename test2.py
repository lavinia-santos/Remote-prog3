...
def calculate_angle_bending_gradient(file, atom_types):
    ...
    for angle in angles:
        ...
        if atom_types[atom2_number] == 'C':
            ...
            # print("rba:", rba)
            # print("rbc:", rbc)
            # print("p (cross product):", p)
            # print("dot product components:", np.dot(rab**2, p))


            gradient2 = (np.cross(-rba,p)/(rab_norm2 * p_norm))+(np.cross(rbc,p)/(rbc_norm2 * p_norm))
            # gradient2 = (np.cross(-rba,p)/(np.dot(rab2,p)))+(np.cross(rbc,p)/(np.dot(rbc2,p)))
...
if __name__ == "__main__":
    # debug('methane')  # Replace 'ethane' with the name of your file
    # print(calculate_bond_stretching_gradient('ethane', read_input('ethane', dev=True)[5]))
